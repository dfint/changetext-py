...
def test_consecutive_tags():
    assert (change_text('Она   гражданин   <gent>   <capitalize>    Ochre   Girders.   Она   член   <gent>')
            == 'Она   гражданин Ochre   Girders.   Она   член')
    changetext.init()
    assert (change_text('Она  гражданин  <gent>  <capitalize>  Livid Dyes.  Она  член <gent>  <capitalize>')
            == 'Она  гражданин Livid Dyes.  Она  член')


def test_tag_spaces():
    changetext.init()
    assert (change_text('Lyrical Wisp. По  возможности она предпочитает употреблять<accs>  ячий сыр и')
            == 'Lyrical Wisp. По  возможности она предпочитает употреблять ячий сыр и')
    assert (change_text('вино из плодов восковницы. Она совершенно не выносит<accs> комары.')
...
def test_commas():
    assert (change_text('летящий {+железный болт+} бьёт <accs> индюк в <accs> голова, разрывая <accs>')
